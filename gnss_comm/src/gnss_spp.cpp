...
namespace gnss_comm
{
    ...
    std::vector<SatStatePtr> sat_states_rtklib(const std::vector<ObsPtr> &obs, 
        const std::vector<EphemBasePtr> &ephems)
    {
        ...
        for (size_t i = 0; i < num_obs; ++i)
        {
            ...
            sat_state->tgd = this_obs->Utgd;
            sat_state->sat_id = sat;
            sat_state->ttx    = this_obs->time;
            sat_state->pos    = Eigen::Vector3d(this_obs->Usat_pos_x, this_obs->Usat_pos_y,this_obs->Usat_pos_z);
            sat_state->vel    = Eigen::Vector3d(this_obs->Uvel_x, this_obs->Uvel_y,this_obs->Uvel_z);
            sat_state->dt     = this_obs->Udt;
            sat_state->ddt    = this_obs->Uddt;
        }
        ...
        const std::vector<EphemBasePtr> &ephems)
    {
        ...
        for (size_t i = 0; i < num_obs; ++i)
        {
            ...
            // if (l1_idx < 0)   
            // {
            //     std::cout << "incorrect frequency domain "<< std::endl;
            //     continue;
            // }
        ...
        const std::vector<EphemBasePtr> &ephems, const std::vector<double> &iono_params)
    {
        ...
        /* by weisong */
        std::vector<SatStatePtr> all_sat_states = sat_states_rtklib(valid_obs, valid_ephems);
        ...
        while(num_iter < MAX_ITER_PVT && dx_norm > EPSILON_PVT)
        {
            ...
            for (uint32_t i = 0; i < valid_obs.size(); ++i)
            {
                ...
                if(obs_sys==SYS_GPS) sys_mask[0] = 1;
                else sys_mask[1] = 1;
                  ...
                  Eigen::VectorXd &res, Eigen::MatrixXd &J)
    {
        ...
        for (uint32_t i = 0; i < num_sv; ++i)
        {
            ...
            double dopp_estimated = (sat_state->vel - rcv_state.topLeftCorner<3, 1>()).dot(unit_rv2sv) + 
                    rcv_state(3, 0) + sagnac_term - sat_state->ddt;
            int l1_idx = -1;
            /* by weisong */
            // const double obs_freq = L1_freq(obs[i], &l1_idx);
            // if (obs_freq < 0)   continue;
            const double wavelength = obs[i]->freqs[0]; // obs->freqs[0]
            res(i) = dopp_estimated + obs[i]->dopp[0]*wavelength;
