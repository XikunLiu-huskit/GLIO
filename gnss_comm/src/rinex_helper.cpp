...
namespace gnss_comm
{
    ...
    *-------------------------------------------------------------------------------------*/
    ...
    void rinex2ephems(const std::string &rinex_filepath, std::map<uint32_t, std::vector<EphemBasePtr>> &sat2ephem, std::map<uint32_t,std::vector<EphemPtr>> &sat2ephem2)
    {
        ...
        while(std::getline(ephem_file, line))
        {
            ...
            if (line.find("RINEX VERSION / TYPE") != std::string::npos && line.find("3.02") == std::string::npos)
            {
                LOG(ERROR) << "Only RINEX 3.04 is supported for observation file";
                // std::cout << "Only RINEX 3.04 is supported for observation file"<<std::endl;
                return;
            }
        ...
        while(std::getline(ephem_file, line))
        {
            /* ephemrics for GPS/BeiDou and Galleo satellites */
            if (line.at(0) == 'G' || line.at(0) == 'C' || line.at(0) == 'E')
            {
                ...
                if (sat2ephem2.count(ephem->sat) == 0)
                    sat2ephem2.emplace(ephem->sat, std::vector<EphemPtr>());
                sat2ephem2.at(ephem->sat).push_back(ephem);
                
                // std::cout << "satellite system is -> "<<std::to_string(ephem->sat)<<std::endl;
            }
            /* ephemrics for GLONASS satellites */
            else if (line.at(0) == 'R')
            {
                std::vector<std::string> ephem_lines;
                ephem_lines.push_back(line);
                ...
                // if (sat2ephem2.count(glo_ephem->sat) == 0)
                //     sat2ephem2.emplace(glo_ephem->sat, std::vector<EphemBasePtr>());
                // sat2ephem2.at(glo_ephem->sat).push_back(glo_ephem);
            }
        }
    }

    void findClosestEphem(gtime_t obsTime, std::map<uint32_t, std::vector<EphemBasePtr>> ephemSets, uint32_t sat, EphemBasePtr& ephemBase)
    {
        double timeDiff = 100000000;
        // std::cout << "sat->  " << sat <<std::endl;
        std::vector<EphemBasePtr> ephemSet = ephemSets.at(sat);
        int length = ephemSet.size();
        for(int i = 0; i < length; i++)
        {
            if(timeDiff>time_diff(obsTime, ephemSet[i]->toe))
            {
                ephemBase = ephemSet[i];
                timeDiff = time_diff(obsTime, ephemSet[i]->toe);
            }
        }
        ...
        const std::map<uint8_t, std::vector<std::string>> &sys2type)
    {
        ...
        for (auto type : date_types)
        {
            ...
            /* Weisong: distribute the memory, otherwise segment fault */
            obs->cp.resize(obs->freqs.size());
            obs->psr.resize(obs->freqs.size());
            obs->dopp.resize(obs->freqs.size());
            obs->CN0.resize(obs->freqs.size());
    ...
    void rinex2obs(const std::string &rinex_filepath, std::vector<std::vector<ObsPtr>> &rinex_meas)
    {
        ...
        {
            if (rinex_line.find("RINEX VERSION / TYPE") != std::string::npos && rinex_line.find("3.03") == std::string::npos)
            {
                LOG(ERROR) << "Only RINEX 3.03 is supported for observation file";
                return;
            }
        ...
        while (std::getline(obs_file, rinex_line))
        {
            ...
            for (int i = 0; i < num_obs; ++i)
            {
                ...
                if(sys==SYS_GPS)
                {
                    std::cout << "rinex_line2obs: GPS Satellite  "<<std::endl;
                    meas.emplace_back(obs);
                }
                else if(sys==SYS_BDS)
                {
                    std::cout << "rinex_line2obs: BeiDou Satellite   "<<std::endl;
                    meas.emplace_back(obs);
                }
                else
                {
                    // std::cout << "Unknow!!!!! Satellite   "<<obs->sat << std::endl;
                }
    ...
    void rinex2iono_params(const std::string &rinex_filepath, std::vector<double> &iono_params)
    {
        ...
        if (!(std::getline(file, line) && line.find("RINEX VERSION")  != std::string::npos 
                && line.find("3.02") != std::string::npos))
        {
            LOG(ERROR) << "Only RINEX 3.02 is supported";
            return;
        }
