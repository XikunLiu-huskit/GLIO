...
struct LidarPoseFactorBatchRelativeAutoDiff
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    LidarPoseFactorBatchRelativeAutoDiff(Eigen::Quaterniond delta_q_, Eigen::Vector3d delta_p_): delta_q(delta_q_), delta_p(delta_p_)
    {}
    ...
    bool operator()(const T *p1, const T *p2, const T *p3, const T *p4, T *residuals) const
    {
        ...
        residuals[3] = T(20) * residual2[0];
        residuals[4] = T(20) * residual2[1];
        residuals[5] = T(20) * residual2[2];
    ...
    static ceres::CostFunction *Create(Eigen::Quaterniond delta_q_, Eigen::Vector3d delta_p_)
    {
        return (new ceres::AutoDiffCostFunction<LidarPoseFactorBatchRelativeAutoDiff, 6, 3, 4, 3, 4>(
                new LidarPoseFactorBatchRelativeAutoDiff(delta_q_, delta_p_)));
    }
...
struct LidarPoseFactorAutoDiffBatch
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    LidarPoseFactorAutoDiffBatch(Eigen::Vector3d p_target_): p_target(p_target_)
    {}
    ...
    bool operator()(const T *p1, T *residuals) const
    {
        ...
        Eigen::Matrix<T, 3, 1> residual1 = T(1) * (P1 - p_target);

        residuals[0] = T(1.2) * residual1[0];
        residuals[1] = T(1.2) * residual1[1];
        residuals[2] = T(1.2) * residual1[2];

        return true;
    }

    static ceres::CostFunction *Create(Eigen::Vector3d p_target_)
    {
        return (new ceres::AutoDiffCostFunction<LidarPoseFactorAutoDiffBatch, 3, 3>(
                new LidarPoseFactorAutoDiffBatch(p_target_)));
    }
