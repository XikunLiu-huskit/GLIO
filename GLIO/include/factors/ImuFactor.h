...
/* vel limit FACTORS */
struct bgLimitFactor
{
    bgLimitFactor(double var, double alpha, double sigma, double velLimit)
            :var(var), alpha(alpha), sigma(sigma), velLimit(velLimit) {}
    ...
    bool operator()(const T* stateVi, T* residuals) const
    {
        ...
        if((stateVi[6])<=(lowLimit + T(sigma)))
        {
            residuals[0] = T(alpha) * (lowLimit-stateVi[6]+sigma);
            residuals[0] = residuals[0] / T(var);
        }
        else if((stateVi[6])>=(upplerLimit - T(sigma)))
        {
            residuals[0] = T(alpha) * (stateVi[6] - upplerLimit +sigma);
            residuals[0] = residuals[0] / T(var);
        }
        else
        {
        ...
        if((stateVi[7])<=(lowLimit + T(sigma)))
        {
            residuals[1] = T(alpha) * (lowLimit-stateVi[7]+sigma);
            residuals[1] = residuals[1] / T(var);
        }
        else if((stateVi[7])>=(upplerLimit - T(sigma)))
        {
            residuals[1] = T(alpha) * (stateVi[7] - upplerLimit +sigma);
            residuals[1] = residuals[1] / T(var);
        }
        else
        {
        ...
        if((stateVi[8])<=(lowLimit + T(sigma)))
        {
            residuals[2] = T(alpha) * (lowLimit-stateVi[8]+sigma);
            residuals[2] = residuals[2] / T(var);
        }
        else if((stateVi[8])>=(upplerLimit - T(sigma)))
        {
            residuals[2] = T(alpha) * (stateVi[8] - upplerLimit +sigma);
            residuals[2] = residuals[2] / T(var);
        }
        else
        {
...
/* vel limit FACTORS */
struct baLimitFactor
{
    baLimitFactor(double var, double alpha, double sigma, double velLimit)
            :var(var), alpha(alpha), sigma(sigma), velLimit(velLimit) {}
    ...
    bool operator()(const T* stateVi, T* residuals) const
    {
        ...
        /* residual for x direction */
        if((stateVi[3])<=(lowLimit + T(sigma)))
        {
            residuals[0] = T(alpha) * (lowLimit-stateVi[3]+sigma);
            residuals[0] = residuals[0] / T(var);
        }
        else if((stateVi[3])>=(upplerLimit - T(sigma)))
        {
            residuals[0] = T(alpha) * (stateVi[3] - upplerLimit +sigma);
            residuals[0] = residuals[0] / T(var);
        }
        else
        {
        ...
        /* residual for y direction */
        if((stateVi[4])<=(lowLimit + T(sigma)))
        {
            residuals[1] = T(alpha) * (lowLimit-stateVi[4]+sigma);
            residuals[1] = residuals[1] / T(var);
        }
        else if((stateVi[4])>=(upplerLimit - T(sigma)))
        {
            residuals[1] = T(alpha) * (stateVi[4] - upplerLimit +sigma);
            residuals[1] = residuals[1] / T(var);
        }
        else
        {
        ...
        /* residual for z direction */
        if((stateVi[5])<=(lowLimit + T(sigma)))
        {
            residuals[2] = T(alpha) * (lowLimit-stateVi[5]+sigma);
            residuals[2] = residuals[2] / T(var);
        }
        else if((stateVi[5])>=(upplerLimit - T(sigma)))
        {
            residuals[2] = T(alpha) * (stateVi[5] - upplerLimit +sigma);
            residuals[2] = residuals[2] / T(var);
        }
        else
        {
