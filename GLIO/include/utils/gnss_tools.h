...
/**
  ...
  */
  ...
  /**
   * @brief  weighted least square for signle point positioning (GPS BDS GLO)
   * @param eAllSVPositions ((n,4) prn, sx, sy, sz, )     eAllSVPositions ((n,3) PRN CNO Pseudorange)
   * @return eWLSSolution 6 unknowns with three clock bias variables
   @ 
  */
  Eigen::MatrixXd WeightedLeastSquare(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, nlosExclusion::GNSS_Raw_Array GNSS_data, std::string method){
    ...
    map<string, int> GREC_data, GREC_idx;
      nlosExclusion::GNSS_Raw_Array GNSS_data_valid;
      GNSS_data_valid.GNSS_Raws.clear();
    ...
    for (int idx = 0; idx < eAllMeasurement.rows(); idx++){
      for (int jdx = 0; jdx < eAllSVPositions.rows(); jdx++){
        if (int(eAllMeasurement(idx, 0)) == int(eAllSVPositions(jdx, 0))){
            if (PRNisGPS(int(eAllMeasurement(idx, 0)))){
                GREC_data["GPS"]++;
                validNumMeasure++;
                validMeasure.push_back(int(eAllMeasurement(idx, 0)));
            }
            else if (PRNisGLONASS(int(eAllMeasurement(idx, 0))))
            {
                GREC_data["GLO"]++;
                validNumMeasure++;
                validMeasure.push_back(int(eAllMeasurement(idx, 0)));
            }
            else if (PRNisBeidou(int(eAllMeasurement(idx, 0))))
            {
                GREC_data["BDS"]++;
                validNumMeasure++;
                validMeasure.push_back(int(eAllMeasurement(idx, 0)));
            }
            else if (PRNisGAL(int(eAllMeasurement(idx, 0))))
            {
                GREC_data["GAL"]++;
                validNumMeasure++;
                validMeasure.push_back(int(eAllMeasurement(idx, 0)));
    ...
    int unknown_num = 3;
    if (GREC_data["GPS"]>4) {
        validNumMeasure += GREC_data["GPS"];
        unknown_num++;
    }
    if (GREC_data["GLO"]>4) {
        validNumMeasure += GREC_data["GLO"];
        unknown_num++;
    }
    if (GREC_data["BDS"]>4) {
        validNumMeasure += GREC_data["BDS"];
        unknown_num++;
    }
    if (GREC_data["GAL"]>4) {
        validNumMeasure += GREC_data["GAL"];
        unknown_num++;
    }

      int sys_idx = 3;
      if (GREC_data["GPS"]>4) {
          GREC_idx["GPS"] = sys_idx;
          sys_idx++;
      }
      if (GREC_data["GLO"]>4) {
          GREC_idx["GLO"] = sys_idx;
          sys_idx++;
      }
      if (GREC_data["BDS"]>4) {
          GREC_idx["BDS"] = sys_idx;
          sys_idx++;
      }
      if (GREC_data["GAL"]>4) {
          GREC_idx["GAL"] = sys_idx;
          sys_idx++;
      }
    ...
    for (int idx = 0; idx < eAllMeasurement.rows(); idx++){
      for (int jdx = 0; jdx < eAllSVPositions.rows(); jdx++){
        if (int(eAllMeasurement(idx, 0)) == int(eAllSVPositions(jdx, 0))){
          ...
          int prn_ = int(eAllMeasurement(idx, 0));
          if (GREC_data["GPS"]>4 && PRNisGPS(int(eAllMeasurement(idx, 0)))){
              validMeasurement.block<1, 3>(valid_meas_idx, 0) = eAllMeasurement.block<1, 3>(idx, 0);
              GNSS_data_valid.GNSS_Raws.push_back(GNSS_data.GNSS_Raws[idx]);
              sat_valid = true;
          }
          else if (GREC_data["GLO"]>4 && PRNisGLONASS(int(eAllMeasurement(idx, 0))))
          {
              validMeasurement.block<1, 3>(valid_meas_idx, 0) = eAllMeasurement.block<1, 3>(idx, 0);
              GNSS_data_valid.GNSS_Raws.push_back(GNSS_data.GNSS_Raws[idx]);
              sat_valid = true;
          }
          else if (GREC_data["BDS"]>4 && PRNisBeidou(int(eAllMeasurement(idx, 0))))
          {
              validMeasurement.block<1, 3>(valid_meas_idx, 0) = eAllMeasurement.block<1, 3>(idx, 0);
              GNSS_data_valid.GNSS_Raws.push_back(GNSS_data.GNSS_Raws[idx]);
              sat_valid = true;
          }
          else if (GREC_data["GAL"]>4 && PRNisGAL(int(eAllMeasurement(idx, 0))))
          {
              validMeasurement.block<1, 3>(valid_meas_idx, 0) = eAllMeasurement.block<1, 3>(idx, 0);
              GNSS_data_valid.GNSS_Raws.push_back(GNSS_data.GNSS_Raws[idx]);
              sat_valid = true;
          }
          if (sat_valid) valid_meas_idx++;
        }
      }
    }

      Eigen::MatrixXd eWLSSolution;
      eWLSSolution.resize(unknown_num, 1);
    ...
    for (int idx = 0; idx < validMeasurement.rows(); idx++){
      ...
      if(method == "WLS")
      {
          weight_matrix = cofactorMatrixCal_WLS(GNSS_data_valid, method); //goGPS weighting;
      }
      else if(method == "R-WLS")
      {
          weight_matrix = cofactorMatrixCal_R_WLS(GNSS_data_valid, method); //goGPS weighting;
      }
      else if (method == "LS")
      {
          weight_matrix = cofactorMatrixCal_WLS(GNSS_data_valid, method); //goGPS weighting;
    ...
    while (!bWLSConverge)
    {
      ...
      for (int idx = 0; idx < iNumSV; idx++){
        ...
        for (int i = 3; i<unknown_num; i++) {
            eH_Matrix(idx, i) = 0;
        }

        if (PRNisGPS(prn)){
          eH_Matrix(idx, GREC_idx["GPS"]) = 1;
        }
        else if (PRNisGLONASS(prn))
        {
          eH_Matrix(idx, GREC_idx["GLO"]) = 1;
        }
        else if (PRNisBeidou(prn))
        {
          eH_Matrix(idx, GREC_idx["BDS"]) = 1;
        }
        else if (PRNisGAL(prn))
        {
          eH_Matrix(idx, GREC_idx["GAL"]) = 1;
        }

        // Making delta pseudorange
        double rcv_clk_bias = 0;
        if (PRNisGPS(prn)){
          rcv_clk_bias = eWLSSolution(GREC_idx["GPS"]);
        }
        else if (PRNisGLONASS(prn))
        {
          rcv_clk_bias = eWLSSolution(GREC_idx["GLO"]);
        }
        else if (PRNisBeidou(prn))
        {
            rcv_clk_bias = eWLSSolution(GREC_idx["BDS"]);
        }
        else if (PRNisGAL(prn))
        {
            rcv_clk_bias = eWLSSolution(GREC_idx["GAL"]);
        }
        eDeltaPr(idx, 0) = pr - dGeoDistance - rcv_clk_bias;
      }
  ...
  /**
    ...
    * @return eWLSSolution 5 unknowns with two clock bias variables
    @
   */
  Eigen::MatrixXd WeightedLeastSquareGPSBDS(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, nlosExclusion::GNSS_Raw_Array GNSS_data, std::string method){
